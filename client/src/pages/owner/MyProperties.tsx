        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {properties.map((p) => (
            <div key={p.id} className="card overflow-hidden">
              ...
              <div className="p-4">
                <Link to={`/properties/${p.id}`} className="font-semibold text-slate-800 hover:text-brand-600">{p.title}</Link>
                ...
                <div className="mt-3 flex items-center justify-between border-t border-slate-100 pt-3 text-xs text-slate-400">
                  ...
                  <div className="flex gap-1">
                    <Link to={`/owner/properties/${p.id}/edit`} className="rounded-lg p-1.5 text-slate-500 hover:bg-slate-100 hover:text-brand-600"><Pencil size={16} /></Link>
                    <button onClick={() => remove(p.id)} className="rounded-lg p-1.5 text-slate-500 hover:bg-red-50 hover:text-red-600"><Trash2 size={16} /></button>
                  </div>
                </div>
              </div>
            </div>
