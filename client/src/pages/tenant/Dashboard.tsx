...
const COLORS = ['#f2911a', '#10b981', '#ef4444', '#c0c1d2'];
...
export default function TenantDashboard() {
  ...
  if (!data) return (
    <div>
      <PageHeader title="Dashboard" subtitle="Loading your rentals…" />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{[...Array(6)].map((_, i) => <StatSkeleton key={i} />)}</div>
    </div>
  );
    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-3">
        <div className="card p-6 lg:col-span-2">
          ...
          <p className="text-sm text-ink-400">Rent paid over the last 6 months</p>
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.paidHistory}>
                <defs>
                  <linearGradient id="pay" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#6538ea" stopOpacity={0.3} />
                    <stop offset="95%" stopColor="#6538ea" stopOpacity={0} />
                  </linearGradient>
                </defs>
                ...
                <Area type="monotone" dataKey="value" stroke="#6538ea" strokeWidth={2} fill="url(#pay)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="card p-6">
          <h3 className="font-semibold text-ink-900">Applications</h3>
          <p className="text-sm text-ink-400">Status breakdown</p>
        ...
        <div className="card mt-6 overflow-hidden">
          <div className="flex flex-col gap-4 p-6 sm:flex-row sm:items-center">
            ...
            <div className="flex-1">
              ...
              <h3 className="mt-2 text-lg font-semibold text-ink-900">{rental.property?.title}</h3>
              <p className="text-sm text-ink-500">{rental.property?.address}, {rental.property?.city}</p>
