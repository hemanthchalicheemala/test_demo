  ...
  return (
    <div>
      ...
      <div className="grid gap-8 lg:grid-cols-3">
          ...
          <div className="card mt-6 p-6">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-1 flex items-center gap-1 text-slate-500"><MapPin size={16} /> {property.address}, {property.city}</p>
              </div>
              <span className="rounded-lg bg-slate-100 px-3 py-1 text-sm font-medium text-slate-600">{property.propertyType}</span>
            </div>
            ...
            {amenities.length > 0 && (
              <div className="mt-6">
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {amenities.map((a) => (
                    <span key={a} className="flex items-center gap-2 text-sm text-slate-600">
        ...
        <div className="space-y-6">
          <div className="card p-6">
            ...
            <p className="text-3xl font-bold text-brand-600">{currency(property.monthlyRent)}<span className="text-base font-normal text-slate-400">/mo</span></p>
            <p className="mt-1 text-sm text-slate-500">Security deposit: {currency(property.securityDeposit)}</p>
          ...
          {property.owner && (
            <div className="card p-6">
              <h3 className="font-semibold text-slate-800">Listed by</h3>
              <div className="mt-3 flex items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-slate-700">{property.owner.name}</p>
      ...
      <Modal open={showRequest} onClose={() => setShowRequest(false)} title="Send Rental Request">
        <p className="text-sm text-slate-500">Introduce yourself to the owner and explain why you'd be a great tenant.</p>
